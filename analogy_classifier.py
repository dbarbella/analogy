...
analogy_file_name = "\\test_extractions\\bc_samples.txt"
non_analogy_file_name = "\\test_extractions\\bc_grounds.txt"

def get_analogy_indicator(text):
    # Returns the first analogy string in the text if found one, empty string
    # otherwise.s
    for item in analogy_string_list:
        pattern = " ".join(item)
        if pattern != "as as":
            if pattern in text: return {"analogy_indicator" : pattern}
        else:
            ptrn = "as .*\ as"
            match = re.search(ptrn, text)
            if match != None: return {"analogy_indicator": pattern}

    return {"analogy_indicator" : ""}
...
def get_analogy_string(text):
    ...
    for pattern in analogy_string_list:
        ...
        if match_index != -1:
            ...
            return {"analogy_indicator:" : tuple(pattern)}
...
def get_pos_tags(text):
    ...
    pos_tags = tuple([word[1] for word in nltk.pos_tag(tokens)])
...
non_analogy_list = get_list(non_analogy_file_name)[: 150]
...
feature_sets = [(get_pos_tags(text), label) for (text, label) in samples]
